...
const SolicitudesPendientes: React.FC<Props> = ({ companyId }) => {
    ...
    const manejarAccion = async (id: number, accion: "aceptar" | "rechazar") => {
        try {
            if (accion === "aceptar") await acceptRequest(id);
                ...
                prev.map((s) =>
                    s.id === id
                        ? {
                            ...s,
                            request_status:
                                accion === "aceptar" ? "accepted" : "rejected",
                        }
                        : s
                )
            );
