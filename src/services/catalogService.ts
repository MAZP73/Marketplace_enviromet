...
export const getAllExcesses = async (): Promise<Excess[]> => {
  const res = await catalogApi.get("/excedentes");
  return res.data;
};

export const createExcess = async (excess: Excess): Promise<Excess> => {
  const res = await catalogApi.post("/excedentes", excess);
  return res.data;
};

export const reserveExcess = async (id: number): Promise<Excess> => {
  const res = await catalogApi.patch(`/excedentes/reservar/${id}`);
  return res.data;
};

export const getExcessById = async (id: number): Promise<Excess> => {
  const res = await catalogApi.get(`/excedentes/excedente/${id}`);
  return res.data;
};


export const getGreeting = async (): Promise<{ message: string; status: string }> => {
  const res = await catalogApi.get("/excedentes/greeting");
  return res.data;
};
